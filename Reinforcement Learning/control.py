...
###### BEGIN YOUR CODE ######
no_of_transition=np.zeros((NUM_STATES,NUM_STATES,2))
val=np.random.rand(NUM_STATES,1)*0.1
prob_of_transition=np.ones((NUM_STATES,NUM_STATES,2))/NUM_STATES
no_of_reward=np.zeros((NUM_STATES,2))
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    score_0=np.dot(prob_of_transition[state,:,0],val)
    score_1=np.dot(prob_of_transition[state,:,1],val)
    if score_0>score_1:
        action=0
    ...
    no_of_transition[state,new_state,action]+=1
    no_of_reward[new_state,0]=no_of_reward[new_state,0]+R
    no_of_reward[new_state,1]=no_of_reward[new_state,1]+1
    ###### END YOUR CODE ######
    ...
    if new_state == NUM_STATES - 1:
        ...
        ###### BEGIN YOUR CODE ######
        for act in range(2):
            for sta in range(NUM_STATES):
                if sum(no_of_transition[sta,:,act])>0:
                    prob_of_transition[sta,:,act]=no_of_transition[sta,:,act]/sum(no_of_transition[sta,:,act])

        for sta in range(NUM_STATES):
            if no_of_reward[sta,1]>0:
                reward[sta]=no_of_reward[sta,0]/no_of_reward[sta,1]
        ###### END YOUR CODE ######
        ...
        ###### BEGIN YOUR CODE ######
        new_val=np.zeros((NUM_STATES,1))
        i=0
        while True:
            i+=1
            for sta in range(NUM_STATES):
                val_0=np.dot(prob_of_transition[sta,:,0],val)
                val_1=np.dot(prob_of_transition[sta,:,1],val)
                new_val[sta]=max(val_0,val_1)
            new_val=reward+GAMMA*new_val
            er=np.amax(abs(val[:-1]-new_val[:-1]))
            print('vector--difference %f',abs(val[:]-new_val[:]))
            print('max difference %f',er)
            print('max value %f',max(val))
            val=new_val
            if er<TOLERANCE:
                break


        consecutive_no_learning_trials+=1 if i==1 else 0
        ###### END YOUR CODE ######
